...
| `migration`   | no       | `migration`        | `{ command, args?, shell? }` run once the service is healthy (and after the init service, when present). `shell` opts the command into a shell (`true` for `/bin/sh -c`, or a string such as `zsh -i` for an interactive one); omitted, it runs as argv, so `&&`, `;`, globs and `$VAR` are literal arguments. In shell mode `args` are appended to the command line the shell interprets rather than to an argv. |
