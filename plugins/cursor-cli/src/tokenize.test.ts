...
describe("cursor-cli extra-args tokenizer (APCC-FR-012)", () => {
  it("splits a plain argument string into discrete argv tokens (AP-TC-088)", () => {
    expect(tokenize("--search --profile work")).toEqual(["--search", "--profile", "work"]);
  });
  ...
  it("keeps a double-quoted run together as one token and strips the quotes (AP-TC-088)", () => {
    expect(tokenize('-c "notify=[]"')).toEqual(["-c", "notify=[]"]);
  });
  ...
  it("never interprets shell metacharacters: the AP-TC-089 payload stays literal", () => {
    // `;` starts no second command, `$HOME` is not expanded, and `$(whoami)` is
    // not substituted. Every character survives into its own argv token.
