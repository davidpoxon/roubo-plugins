 ...
 * tokens (APCC-FR-012, and the parent's AP-FR-017, AP-TC-088).
 *
 * A copy of the Codex plugin's tokenizer, itself a copy of the Claude Code
 * plugin's, deliberately duplicated rather than shared: there is no shared
 * workspace here, and the Cursor CLI architecture keeps extracting it out of
 * scope. Each copy carries its own test.
 *
...
export function tokenize(extraArgs: string): string[] {
  ...
  for (let i = 0; i < extraArgs.length; i += 1) {
    ...
    if (char === "\\") {
      ...
      if (next === undefined) {
        throw new Error(
          "cursor-cli agent plugin: the additional CLI arguments end with a dangling backslash escape.",
        );
      }
      current += next;
      started = true;
...
function unbalanced(quote: string): Error {
  return new Error(
    `cursor-cli agent plugin: the additional CLI arguments contain an unbalanced ${quote === "'" ? "single" : "double"} quote.`,
  );
}
