...
/**
 * Directly probes `GET /repos/{owner}/{repo}` for a single repo so the host can
 * tell *why* a configured repo is missing from `listSourceCandidates`. The user
 * repo listing silently omits repos blocked by an org's OAuth App access
 * restrictions (HTTP 200, repo simply absent), so membership in that list can't
 * distinguish "no such repo" from "org has not approved Roubo". A direct fetch
 * surfaces the real 403/404, which `classifyGitHubError` on the host turns into
