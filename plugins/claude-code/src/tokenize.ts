...
export function tokenize(extraArgs: string): string[] {
  ...
  for (let i = 0; i < extraArgs.length; i += 1) {
    ...
    if (char === "\\") {
      ...
      if (next === undefined) {
        throw new Error(
          "claude-code agent plugin: the additional CLI arguments end with a dangling backslash escape.",
        );
      }
      current += next;
      started = true;
...
function unbalanced(quote: string): Error {
  return new Error(
    `claude-code agent plugin: the additional CLI arguments contain an unbalanced ${quote === "'" ? "single" : "double"} quote.`,
  );
}
