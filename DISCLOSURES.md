...
- **Agent plugins** (`@roubo/plugin-claude-code`) map the user's configured settings to the command-line arguments the host application uses to launch an AI coding agent CLI the user has installed themselves, and declare the permission rules and local notification-hook registration the host writes into a settings file inside the bench workspace (rules are merged with the file's existing keys; the notification-hook entry is replaced). The plugin emits a declarative launch descriptor and nothing else: the host performs every write, and the plugin spawns no process, makes no network request, and performs no inference.
